...
- **Agent / docs:** prefer **uv** for all Python CLI (`uv run`, `uv add`, `uvx`); `.cursor/rules/uv-cli.mdc` + tests rule updated; water sensor uses **`flow_sensor/`** + `uv sync --extra pi`; pytest includes **`flow_sensor_math`** coverage.
